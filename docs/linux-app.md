...
| LLM engine | **Ollama is never bundled in the artifact.** Probe-adopt a reachable/system ollama first; else download (pinned, sha256-verified) to `~/.sotto/runtime/ollama/` on the first-run download screen, unpruned initially. | The Linux tarball is ~1.34 GB (bundles CUDA libs) — bundling would quintuple the artifact. Users download models at first run anyway. CPU-only pruning happens after live measurements (which libs a CPU box actually loads). |
| Updater | Reuse `update.py`'s pure tier (parse/evaluate/due/mark_checked, scheduled loop, single-flight lock, download-progress loop) with Linux backends: .deb installs via `pkexec sotto-perms install-update` (which uses `apt-get install ./file` so new Depends resolve); AppImage **self-replaces** `$APPIMAGE` and re-execs (no root, no prompt). Asset naming extends the existing `Sotto-<ver>-<arch>.<ext>` convention. | Same update UX as macOS. Source checkouts keep `run.sh`'s git-pull; `bundle_type()` returns None there so the updater stays silent. zsync deltas deferred (unproven savings at ~1 GB). |
...
| Arch | **amd64 only** initially; arm64 Linux deferred (`asset_suffix()` returns None there → updater silent). | No test hardware; the friend's machine is amd64. |
...
3. **L3 — PyInstaller onedir + CI smoke.** `linuxapp/{build_app.sh,sotto.spec,
   ...
   pass).
4. **L4 — Ollama runtime resolution.** `sotto/ollama_runtime.py` (resolve:
...
5. **L5 — Linux first-run: checks + root helper + Tk windows.**
...
6. **L6 — .deb + release pipeline** (the big one). `make_deb.sh` + packaging
   ...
   ydotoold user-unit status if GNOME-Wayland.
7. **L7 — Tray (best-effort).** `sotto/tray_linux.py` via pystray; Quit does
   full clean shutdown incl. `llm_server.shutdown`. Unit: `test_tray_menu`.
   Friend: tray visible on Ubuntu; Quit exits; tray-less GNOME still works.
8. **L8 — Updater Linux backend.** `sotto/update_linux.py` (`bundle_type()`,
   ...
   Updates → Update Now → polkit prompt → relaunches as vN+1; repeat the
   AppImage self-replace.
9. **L9 — AppImage.** `make_appimage.sh` + AppRun + static runtime + embedded
   ...
   full setup, dictation; delete the AppImage → permissions persist.
10. **L10 — Docs + release dry run.** This file's milestones marked done with
